...
public class StringHelper {
    ...
    public static String capWords(String value) {
        ...
        for (String val : splitString) {
            StringBuilder stringBuilder = word.append(capitalize(val));
            if (index < splitString.length - 1) stringBuilder.append(" ");
            index++;
        }
    ...
    private static String covertToHex(byte[] data) {
    ...
    private static String converToSHa512(String text)
            throws NoSuchAlgorithmException, UnsupportedEncodingException {
        MessageDigest md = MessageDigest.getInstance("SHa-512");
        md.update(text.getBytes("iso-8859-1"), 0, text.length());
        byte[] hash = md.digest();
        return covertToHex(hash);
    }
