    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, MarketPaginationItem> {
        return try {
            ...
            if (response.code() == 200) {
                ...
                val nextPageNumber = if (marketPaginationItem.data.size < pageSize) null else pageNumber + 1
                val prevPageNumber = if (pageNumber > 1) pageNumber - 1 else null
                return LoadResult.Page(marketPaginationItem.data, prevPageNumber, nextPageNumber)
            } else {
